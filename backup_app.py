...
def display_monthly_view(user, year, quarter, month):
    ...
    if goals:
        ...
        /* Unified Header Styling (All headers in same light blue) */
        .excel-table th,
        .excel-table .section-header {
            background-color: #3B82F6; /* Uniform light blue for all headers */
            color: #ffffff;
            padding: 10px;
            text-align: center;
        ...
        /* Target columns - soft blue tint */
        .excel-table .target-col {
            background-color: #F3F8FF; /* very light blue */
        }

        /* Achievement columns - soft green tint */
        .excel-table .achievement-col {
            background-color: #F6FFF8; /* very light green background */
        }

        /* Monthly Achievement - slightly bolder green */
        .excel-table .monthly-achievement-col {
            background-color: #E9FCEB; /* light greenish */
            font-weight: bold;
        }

        /* Remarks columns - soft cream background */
        ...
        html_table = '<div class="excel-table"><table>'
        
        # Header Row 1 - Main Categories
        html_table += '<thead><tr>'
        html_table += '<th rowspan="2">Vertical</th>'
        html_table += '<th rowspan="2">Goal Title</th>'
        html_table += '<th rowspan="2">KPI</th>'
        html_table += '<th rowspan="2">Monthly Target</th>'
        html_table += '<th rowspan="2">Start Date</th>'
        html_table += '<th rowspan="2">End Date</th>'
        html_table += '<th colspan="4" class="section-header">Weekly Target</th>'
        html_table += '<th colspan="4" class="section-header">Weekly Achievement</th>'
        html_table += '<th rowspan="2" class="section-header">Monthly Achievement</th>'
        html_table += '<th colspan="4" class="section-header">Remarks</th>'
        html_table += '</tr>'
        ...
        # Data Rows
        html_table += '<tbody>'
        for goal, (_, row) in zip(goals, df.iterrows()):
            html_table += '<tr>'
            html_table += f'<td>{row["Vertical"]}</td>'
            
            # ✅ FIXED: Show description in a better modal
            goal_desc_clean = goal.get('goal_description', 'No description available')
            html_table += f'''<td>
                <button style="background: none; border: none; color: #3B82F6; text-decoration: underline; 
                    font-weight: bold; cursor: pointer; padding: 0; font-size: inherit;"
                    onclick="document.getElementById('desc-modal-{goal['goal_id']}').style.display='block'">
                    {row["Goal Title"]}
                </button>
                <div id="desc-modal-{goal['goal_id']}" style="display: none; position: fixed; z-index: 1000; 
                    left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.4);">
                    <div style="background-color: white; margin: 10% auto; padding: 20px; border-radius: 10px; 
                        width: 60%; max-width: 600px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                            <h3 style="margin: 0; color: #1E3A8A;">Goal Description</h3>
                            <button onclick="document.getElementById('desc-modal-{goal['goal_id']}').style.display='none'"
                                style="background: #ef4444; color: white; border: none; border-radius: 5px; 
                                padding: 5px 15px; cursor: pointer; font-weight: bold;">✕</button>
                        </div>
                        <p style="color: #333; line-height: 1.6; white-space: pre-wrap;">{goal_desc_clean}</p>
                    </div>
                </div>
            </td>'''
            
            html_table += f'<td>{row["KPI"]}</td>'
            html_table += f'<td style="text-align: center;">{row["Monthly Target"]}</td>'
            html_table += f'<td style="text-align: center;">{row["Start Date"]}</td>'
            html_table += f'<td style="text-align: center;">{row["End Date"]}</td>'
            
            # Weekly Targets
            for week in range(1, 5):
                html_table += f'<td class="target-col" style="text-align: center;">{row[f"Week {week} Target"]}</td>'
            ...
            for week in range(1, 5):
                ...
                html_table += f'<td class="achievement-col" style="text-align: center; color: {color}; font-weight: bold;">{achievement}</td>'
            ...
            html_table += f'<td class="monthly-achievement-col" style="text-align: center; color: {monthly_color};">{monthly_achievement}</td>'
            
            # Weekly Remarks
            for week in range(1, 5):
                remarks = row[f"Week {week} Remarks"]
                html_table += f'<td class="remarks-col">{remarks if remarks else "-"}</td>'
        ...
        # ✅ CHANGED: Show Update tab for all users, Edit/Delete only for HR/Manager
        if st.session_state.get('viewing_employee_year'):
            # Viewing employee goals - show all tabs for HR/Manager
            ...
            # HR/Manager viewing own goals - show all tabs
            action_tab1, action_tab2, action_tab3 = st.tabs([
                "📝 Update Achievements",
                "✏️ Edit Goal",
                "🗑️ Delete Goal"
            ...
            # ✅ NEW: Employee viewing own goals - only update tab
        ...
        with action_tab1:
            ...
            # Calculate total monthly achievement
            ...
            if st.button("💾 Save Achievements", use_container_width=True, key="save_achievements"):
                # Update monthly goal with weekly achievements and total monthly achievement
                updates = {
        ...
        # ===== EDIT GOAL TAB =====
        if action_tab2:  # ✅ Only show if tab exists (HR/Manager)
            with action_tab2:
        ...
        # ===== DELETE GOAL TAB =====
        if action_tab3:  # ✅ Only show if tab exists (HR/Manager)
            with action_tab3:
