...
class Database:
    ...
    def create_goal(self, goal_data: Dict[str, Any]) -> bool:
        """Create a new goal"""
        try:
            if 'created_by' not in goal_data:
                goal_data['created_by'] = goal_data['user_id']
            self.supabase.table('goals').insert(goal_data).execute()
            return True
        except Exception as e:
    ...
    def get_user_goal_stats(self, user_id: str, year: Optional[int] = None) -> Dict:
        ...
        try:
            ...
            avg_progress = (total_achievement / total_target * 100) if total_target > 0 else 0
